...
class MainWindow(TkinterDnD.Tk if is_dnd_compatible else tk.Tk):
    ...
    def show_file_dialog(self, text='Select Audio files', dialoge_type=None, initialdir=None, initialfile=None):
        parent_win = root
    ...
    def input_select_filedialog(self):
        ...
        initial_dir = os.path.dirname(self.inputPaths[0]) if self.inputPaths else self.lastDir
        initial_file = os.path.basename(self.inputPaths[0]) if self.inputPaths else None
        paths = self.show_file_dialog(dialoge_type=MAIN_MULTIPLE_FILE, initialdir=initial_dir, initialfile=initial_file)
    ...
    def export_select_filedialog(self):
        ...
        initial_dir = self.export_path_var.get() if self.export_path_var.get() else self.lastDir
        path = self.show_file_dialog(dialoge_type=CHOOSE_EXPORT_FIR, initialdir=initial_dir)
        ...
        try:
            ...
            for dir in DIRECTORIES:
                for temp_file in os.listdir(dir):
                    if temp_file.endswith(EXTENSIONS) and temp_file != 'requirements.txt': # HACK: Exclude requirements.txt to prevent accidental deletion
